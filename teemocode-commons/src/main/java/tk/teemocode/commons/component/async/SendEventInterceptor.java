package tk.teemocode.commons.component.async;
...
public class SendEventInterceptor {
	...
	/**
	 * 切入点：tk.teemocode.product包下的所有public方法执行(测试用)
	 */
	@Pointcut("execution(public * tk.teemocode.web..action..*.*(..))")
	private void allAction() {
	}

	/**
	 * 切入点：tk.teemocode包下的所有带SendEvent注解的public方法执行
	 */
	@Pointcut("execution(@tk.teemocode.commons.component.async.SendEvent * *(..))")
